...
export function validateAuthEnvironment() {
  ...
  try {
    ...
    validateSecret(
      "AUTH_JWT_SECRET",
      process.env.BETTER_AUTH_JWT_SECRET || process.env.AUTH_JWT_SECRET
    );
    ...
    if (!corsOrigins || corsOrigins.trim().length === 0) {
      errors.push(
        "CORS_ORIGINS environment variable is required. Set it to comma-separated list of allowed origins."
      );
    }
  } catch (error) {
